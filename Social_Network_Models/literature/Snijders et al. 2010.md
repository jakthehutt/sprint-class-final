...
inﬂuences on network change, and allow to estimate parameters expressing such inﬂuences, and test
...
they may ﬂourish and perhaps evolve into close relationships, and
...
ates’), and residual random inﬂuences representing unexplained
inﬂuences. Social network research has in recent years paid increas-
...
tative matching, as will be elaborated below. In this way, the models
should be able to give a good representation of the stochastic depen-
...
to a very speciﬁc set of micro-mechanisms – allowing detailed
...
T.A.B. Snijders et al. / Social Networks 32 (2010) 44–60
45
analyses of the properties of these models –, but lack an explicit esti-
...
the expression of a single social theory as reﬂected, e.g., by a simple
...
for other inﬂuences on the network dynamics, and how to estimate
...
cated dependencies between ties such as are generated by transitive
...
are ﬂexibility, allowing to incorporate a wide variety of actor-driven
micro-mechanisms inﬂuencing tie formation; and the availabil-
...
could also be ﬁrms, countries, etc. The ties are supposed to be, in
...
same neighborhood) which inﬂuence the existence of ties. Dyadic
...
such as data requirements and how to test and select the appropri-
...
actor j, denoted i →j, is either present or absent (xij then having val-
...
matrix x = (xij) (self-ties are excluded), where n is the total num-
...
ties as the result of processes where one tie is formed as a reaction
...
at the ﬁrst observation are mutually disconnected form at the
...
a special process speciﬁcally for closed triangles.
...
social context that inﬂuences the probabilities of its own change.
...
Proctor’s (1959) discrete Markov chain model. This is an assump-
tion that will usually not be realistic, but it is difﬁcult to come up
...
can be concluded from the speciﬁcations given below, the actors
...
more than one tie can change at any moment—a principle ﬁrst
proposed by Holland and Leinhardt (1977). This principle decom-
...
only sequentially, via the changing conﬁguration of the whole
...
ﬁrst (continuous-time model) makes sense intuitively; the second
...
ence, which leads to requirements of ﬂexibility (enough parameters
that can be estimated from the data to achieve a good ﬁt between
model and data) and parsimony (not more ﬁne detail in the model
...
Hedström, 2005), to underline that actors are not regarded as sub-
...
This procedure uses the basic principle that the ﬁrst observed net-
...
conditions on the ﬁrst observation. This implies that it is the change
...
the determinants of the network structure at the ﬁrst time point.
2.2. Change determination model
The ﬁrst step in the model is the choice of the focal actor (ego)
...
random inﬂuences. The objective function will depend in practice
on the personal network of the actor, as deﬁned by the network
between the focal actor plus those to whom there is a direct tie (or,
depending on the speciﬁcation, the focal actor plus those to whom
...
bilities of changes are assumed to depend on the personal networks
that would result from the changes that possibly could be made, and
...
and relevant theoretical and ﬁeld-related knowledge. The objective
...
uations where the dependent variable is a choice made from a ﬁnite
set of actions.
2.3. Speciﬁcation of the objective function
...
of the actor. This function is deﬁned on the set of possible states
...
work; the functions ski(x) are the effects, functions of the network
...
tion; and the weights ˇk are the statistical parameters. The effects
...
to select the most appropriate precise speciﬁcation of the endoge-
nous network effects. For example, in the investigation of friendship
...
multiple mathematical speciﬁcations are available (as ‘effects’ ski(x)
...
choosing among these speciﬁcations. It may then be best to use the-
...
and transitive closure in the model, and to choose the best speciﬁ-
cation for transitive closure, by one or several speciﬁc effects, in a
data-driven way.
...
ences.
2.3.1. Basic effects
The most basic effect is deﬁned by the outdegree of actor i, and
...
could be regarded as the balance of beneﬁts and costs of an arbitrary
...
to i–, the costs will usually outweigh the beneﬁts. Indeed, in most
...
cycles, which can be represented here by a negative parameter ˇk
...
gree, or higher outdegree respectively, will have an extra propensity
to form ties to others. These effects can be measured by the inde-
gree of i times i’s outdegree; and, respectively, the outdegree of
...
Therefore these two effects will be difﬁcult, or impossible, to dis-
...
ential attachment by Barabási and Albert (1999) in their models for
...
reﬂected by positive indegree popularity and negative outdegree
...
the better ﬁt by testing the triadic parameters while controlling for
...
settings where degrees reﬂect status of the actors, such preferences
may be argued theoretically based on status-speciﬁc preferences,
...
empirical ﬁt, will have to decide which one is a more meaningful
...
assortativity effects (cf. the relation between main effects and inter-
...
effect, measuring whether actors with higher V values tend to nom-
...
of V.
For a dyadic covariate, i.e., a variable deﬁned for pairs of actors,
...
tions that could be used as effects makes it difﬁcult to give general
...
same nationality has a positive main effect, reﬂecting that it is easier
...
is very extensive, naturally reﬂecting the multitude of possibilities
...
grains of salt, we ﬁrst give some numbers to indicate the sizes of
...
plicated models to be ﬁtted. Between any pair of consecutive waves,
...
if one studies inter-ﬁrm network dynamics, given the time delays
involved for ﬁrms in the planning and executing of their ties to other
ﬁrms, it may be enough to collect data once every year, or even less
frequently.
...
unless the ﬁrst wave has a much lower density than the second
...
network has many more ties than the ﬁrst, one may look instead
...
ties (ranging from weak to strong) were collected, then these num-
...
tributions of the estimates of the parameters ˇk in the objective
...
deﬁned as parameter estimate divided by standard error, to a stan-
...
developed, although Koskinen (2004) presents some ﬁrst steps
...
deleted). The steps can be based on signiﬁcance test for the various
...
value usually works ﬁne. For complicated models, however, the
...
from good model ﬁts, for use as possible initial values later on.
4. Network statistics can be highly correlated just because of
their deﬁnition. This also implies that parameter estimates can
...
hypothesis) including signiﬁcant and/or theoretically relevant
...
separate parameter. The parameters for which a signiﬁcant test
...
structure. Since a satisfactory ﬁt is important for obtaining gen-
...
is a ‘satisfactory’ ﬁt, and how complicated models should be in
practice.
...
the start. Non-signiﬁcant control effects might be dropped pro-
visionally and then tested again as a check in the presumed ﬁnal
model; but one might also retain control effects independent of
their statistical signiﬁcance. We do not think there are unequiv-
...
root speciﬁcation usually works best.
...
is good as a ﬁrst general check to consider how average degree
...
waves ﬁrst, which will show the extent of inhomogeneity in the
...
indegrees should be ‘explainable’ from the model speciﬁcation;
if they are only explainable from earlier observations (‘path
dependence’), they will have a tendency to regress toward the
mean. The model speciﬁcation may be able to explain outliers
by covariates identifying exceptional actors, but also by degree-
related endogenous effects such as the self-reinforcing Matthew
effect mentioned above. It is good as a ﬁrst general check to
...
to represent their outlying behavior. Such an ad hoc model adap-
tation, which may improve the ﬁt dramatically, is better than
...
Knecht (2008). The 26 students were followed over their ﬁrst year
...
erate increase over time. Jaccard coefﬁcients for similarity of ties
between waves are between 0.4 and 0.5, which is somewhat low
(reﬂecting fairly high turnover) but not too low.
...
tion using the procedure explained in Huisman and Steglich (2008).
...
but this simple case was treated here by using structural zeros: start-
ing with the ﬁrst observation moment where this pupil was not a
member of the classroom any more, all incoming and outgoing tie
variables of this pupil were ﬁxed to zero and not allowed to change
in the simulations.
...
these expectations. Structural effects included are reciprocity; tran-
...
mary school reﬂecting relationship history. In addition, several
...
are deﬁned in the simulation model as the expected frequencies,
...
This analysis conﬁrms, for this data set, several of the known
properties of friendship networks: there is a high degree of reci-
procity, as seen in the signiﬁcant reciprocity parameter; there is
segregation according to the sexes, as seen in the signiﬁcant same
...
for transitive closure, as seen in the signiﬁcant effects of transi-
...
mind that these are unstandardized coefﬁcients. Other signiﬁcant
...
also is suggested by the marginally signiﬁcant negative effect of the
...
ated with nomination activity. Also signiﬁcant is the sender effect
...
than the girls.
Rate parameters, ﬁnally, suggest that the amount of friend-
...
obtained by eliminating the non-signiﬁcant effects in a backward
selection procedure. The sex alter effect was retained in spite of
its non-signiﬁcance, because the three sex-related effects belong
together as a representation of sex-related friendship preferences.
One by one, the least signiﬁcant of the insigniﬁcant effects were
...
score-type tests.
to check whether the parameter does not become signiﬁcant upon
...
of Model 0. The parameters dropped due to insigniﬁcance were the
...
parameters, it may be kept in mind that the parameters ˇk in the
objective function are unstandardized coefﬁcients of the statistics
of which the mathematical formulae are given in Appendix A.
The parameters in the objective function can be interpreted in
two ways. In the ﬁrst place, by interpreting this function as the
...
mathematical deﬁnitions are given in Appendix A. In most cases the
...
2 More exactly, the value is 
2/6, the standard deviation of the Gumbel distri-
bution.
...
contribution of the single tie xij to the objective function, consider-
...
the outdegrees reﬂect the importance to the organizations of the
...
the difference is not signiﬁcant (p = 0.13).
...
seems rather large but is not signiﬁcant in view of the small sample
size.
...
related to reciprocity. It was estimated as signiﬁcant and positive,
while the corresponding evaluation function effect of reciprocity
dropped in size and signiﬁcance. To interpret this result, jointly con-
...
j3 as friends, but not the other two. Assume ﬁnally (artiﬁcially, for
...
variable ‘sex’ has centered valuevi = −0.346 for girls, the parameter
...
for a girl–girl tie, ﬁve of the options for i are the following:
...
ues, which are, respectively, e−0.90 = 0.41, 0.59, 3.42, 0.29, and 1.
...
tie, depends also on these numbers for the ‘ego’ girl under consider-
...
lishing a friendship to a boy by (F). Retain the unrealistic simplifying
...
does not inﬂuence probabilities of change. The baseline value of a
...
random inﬂuences, however, that would decrease reciprocity—but
...
may inﬂuence one another (e.g., Friedkin, 1998), and ties will be
...
and of the actor variables, but likewise the actor variables are chang-
...
conﬁguration will be also called the social selection process, while
the dependence of the behavior dynamics on the total network-
behavior conﬁguration will be called the social inﬂuence process.
Both social inﬂuence and social selection can lead to similar-
ity between tied actors, which is often observed. A fundamental
question then is whether this similarity is caused mainly by inﬂu-
ence or mainly by selection, as discussed by Ennett and Bauman
(1994) for smoking behavior and Haynie (2001) for delinquent
behavior.
This combination of selection and inﬂuence can be modeled by
...
the actors’ behavior variables, as speciﬁed in Snijders et al. (2007)
...
sense that the explanation of the actor’s outgoing ties and behav-
...
senting the endogenous social inﬂuence process. Since the network
and the behavior variables both inﬂuence the dynamics of the net-
...
position, etc. The strength of the effects of these functions on behav-
ior choices are represented by the parameters ˇZ
k. The superscript
...
We ﬁrst discuss basic tendencies determining behavior change
that are independent of actor attributes and network position.
A baseline deﬁnition for the evaluation function will be a curve,
depending on the actor’s own behavior zi, that can be loosely inter-
preted as the relative preference for the speciﬁc value zi of the
...
then a linear function sufﬁces, as each function of two values can
...
so that a speciﬁcation will be required that allows the function
...
The linear and quadratic coefﬁcients in this function are called the
linear shape effect and the quadratic shape effect. Note that the latter
is superﬂuous for a dichotomous behavior variable.
...
nism: when the value of the actor’s behavior increases, the further
...
the behavior will become smaller. Conversely, when the coefﬁcient
...
niﬁcantly different from 0, are more often seen than positive values.
When the coefﬁcient for the shape effect is denoted ˇZ
1 and
the coefﬁcient for the quadratic shape effect is ˇZ
2, then the total
contribution of these two effects is ˇZ
1zi + ˇZ
2z2
i . With a negative
coefﬁcient ˇZ
2, this is a unimodal preference function. The max-
imum then is attained for zi = −2ˇZ
1/ˇZ
2, or more precisely, the
integer value within the prescribed range that is closest to this num-
...
which is not the case for all similarity effects as deﬁned below! –,
this will change the location of the maximum but not the unimodal
shape of the function.)
5.1.2. Inﬂuence and position-dependent effects
...
The actor-based model can represent social inﬂuence, i.e., inﬂu-
ence from alters’ behavior on ego’s behavior, in various ways,
because there are several different ways to measure and aggregate
the inﬂuences from different alters. Three different representations
are as follows.
...
total inﬂuence of the alters is the same regardless of the number
...
inﬂuence of the alters is proportional to the number of alters.
...
grounds and/or on the basis of statistical signiﬁcance.
In addition to this type of social inﬂuence, network position itself
...
behavior can be included, representing the inﬂuence of this actor
variable on changes in the behavior. In addition, it is possible that
such a variable will moderate the inﬂuence effect, leading to an
interaction between the variable and the inﬂuence effect.
5.2. Speciﬁcation of models for dynamics of networks and
behavior
...
low for specifying a model of network-behavior co-evolution: weak
...
Since it is more difﬁcult to estimate a model of network-behavior
co-evolution than to estimate a model of only network evolution, it
makes sense ﬁrst to estimate a good model for the dynamics of only
...
model.
It has become clear above that there are several speciﬁcations
...
interpretable. For the social inﬂuence part likewise there are several
possible speciﬁcations, such as average similarity, total similarity,
and average alter.
A difﬁculty is that we often have no clear theoretical clue as to
which of these three speciﬁcations is better in a particular case;
the alternative speciﬁcations are deﬁned by effects that may be
...
inﬂuence will depend on the speciﬁcation chosen. If we do have
prior information as to the best speciﬁcation, then it is preferable
to work with this speciﬁcation. If we do not have such informa-
...
56
T.A.B. Snijders et al. / Social Networks 32 (2010) 44–60
using the ‘most signiﬁcant’ effect without taking into account that
it was chosen exactly because it was the most signiﬁcant, then we
...
number of candidate social inﬂuence effects ‘INF’ on theoretical
...
existence of inﬂuence and selection, not controlling each effect
for the other.
4. Select the effects that are individually most signiﬁcant in either
set, and denote these effects by SEL1 and INF1.
(In this formulation, the effects are selected based on their sig-
niﬁcance when hypothesized to be added to the baseline model.
Another possibility is to select the effects based on the following
two models.)
5. To test inﬂuence effects while controlling for selection, estimate
the model BM + SEL1 (i.e., the baseline model extended with the
most signiﬁcant selection effect) and within this model test all
of the effects INF jointly by a score-type test.
6. To test selection effects while controlling for inﬂuence, estimate
the model BM + INF1 and test all of the effects SEL jointly by a
score-type test.
7. Finally, to estimate a model with inﬂuence and selection, esti-
...
(Knecht, 2008). This variable is deﬁned as a rounded average over
four types of minor delinquency (stealing, vandalism, grafﬁti, and
ﬁghting), measured in each of the four waves of data collection.
The ﬁve-point scale ranged from ‘never’ to ‘more than 10 times’,
...
for network inﬂuence processes playing a role in the spread of delin-
quency through the group deﬁned by the classroom. Analyses were
...
elling social inﬂuence with respect to delinquency (the set INF) are
average similarity, total similarity, and average delinquency alter.
Score tests for the selection and inﬂuence effects tested with the
baseline model as the null hypothesis yielded the following p-
values. For both parts of the model, ﬁrst the results of the overall
...
p
Friendship dynamics
Overall test for social selection (4 d.f.)
< 0.001
Delinquency ego
< 0.001
Delinquency alter
...
Overall test for social inﬂuence (3 d.f.)
0.04
Average similarity
0.03
Total similarity
0.12
Average delinquency alter
0.48
The overall tests show evidence for social selection (p < 0.001)
as well as for social inﬂuence (p < 0.05), when these are not being
...
To test selection while controlling for inﬂuence, the baseline
model was extended with inﬂuence operationalized as average sim-
...
To test inﬂuence while controlling for selection, the base-
line model was extended with selection operationalized as
delinquency similarity, and the three inﬂuence effects (aver-
...
for inﬂuence from pupils on the delinquent behavior of their
friends.
To estimate a model incorporating selection and inﬂuence,
...
ics model, but none of these were signiﬁcant.
...
the delinquency similarity measure; and for inﬂuence from pupils
...
to be inﬂuenced by sex. The model for network dynamics yields
...
0.11
0.014
Outdegree based popularity (sqrt)
−0.52
0.25
0.034
Sex (M) ego
...
Network rate period 1
9.94
2.12
Network rate period 2
10.86
2.00
Network rate period 3
9.39
1.49
Delinquency linear
0.00
...
Delinquency rate period 1
1.50
0.70
Delinquency rate period 2
3.50
2.48
Delinquency rate period 3
2.64
1.39
...
be helpful to reﬂect about equilibrium and out-of-equilibrium
social systems. Equilibrium is understood here not as a ﬁxed
state but as dynamic equilibrium, where changes continue but
may be regarded as stochastic ﬂuctuations without a systematic
...
reached dynamic equilibrium, and then take samples from the pro-
...
far from an equilibrium situation, then it is difﬁcult to see what
...
discernible trends or important ﬂuctuations in average degree, in
...
the random inﬂuences which, without this aspect, would make the
...
if one were to make an analysis of the ﬁrst one by an ERGM and of
...
it would be possible that the ﬁrst observed network shows no tran-
sitive closure at all, but the dynamics does show a transitivity effect;
...
possible that there is a strong transitivity effect at the ﬁrst observa-
...
reﬂections of social rules and regularities than what can be derived
from a single cross-sectional observation. This also is an argument
for not modeling the ﬁrst observation but using it only as an ini-
tial condition for the network dynamics (as mentioned at the end
of Section 2.1). In many situations the ﬁrst observation cannot be
regarded as coming from a process in equilibrium, and then it is
unclear what the ﬁrst observation by itself can tell us about social
...
Further, the models must be ﬂexible enough to represent sev-
...
ther development of ways to assess the goodness of ﬁt of these
models and to diagnose what in the data-model combination may
be mainly responsible for a possible lack of ﬁt. Another issue is the
assessment of the robustness of results with respect to misspeciﬁed
...
models presented here do not ﬁt to a satisfactory degree. A case
in point would be the development of models permitting a more
elaborate temporal dependence than Markovian dependence. All
this should lead to better knowledge about how to ﬁt longitudinal
models to network data, and hence to more reliable results. In this
tutorial we have tried to represent the current knowledge about the
speciﬁcation of longitudinal network models in a concise but more
...
.
(4)
In words: the probability that an actor makes a speciﬁc change
...
the possible new states are z0 −1, z0, and z0 + 1 (unless the ﬁrst or
...
sion.
A.2. Effects
Some formulae for effects sik(x) are as follows. Replacing an
index by a + sign denotes summation over this index. Exogenous
actor covariates are denoted by vi and dyadic covariates by wij.
Reciprocity

j
xijxji,
...
xij(simij −sim),
(18)
where simij = (1 −|vi −vj|/), with  = maxij|vi −vj|;
same V

j
xijI{vi = vj},
...
T.A.B. Snijders et al. / Social Networks 32 (2010) 44–60
59
actor cov. V × transitive triplets
vi

j,h
xijxjhxih,
(23)
Some formulae for behavior effects sZ
ik(x, z) are the following:
linear shape
zI,
(24)
quadratic shape
...
(28)
where simz
ij = (1 −|zi −zj|/Z) with Z = maxij|zi −zj|;
total similarity

j
xij(simz
...
rica 68, 1181–1229.
Batagelj, V., Bren, M., 1995. Comparing resemblance measures. Journal of Classiﬁca-
tion 12, 73–90.
...
deal-making. European Management Review 4, 161–171.
...
35, 843–852.
de Federico de la Rúa, A., 2004. L’Analyse longitudinale de réseaux sociaux totaux
avec SIENA – Méthode, discussion et application, BMS. Bulletin de Méthodologie
Sociologique 84, 5–39.
Doreian, P., Stokman, F.N. (Eds.), 1997. Evolution of Social Networks. Gordon and
Breach Publishers, Amsterdam.
Ennett, S.T., Bauman, K.E., 1994. The contribution of inﬂuence and selection to adoles-
cent peer group homogeneity: the case of adolescent cigarette smoking. Journal
of Personality and Social Psychology 67, 653–663.
Friedkin, N.E., 1998. A Structural Theory of Social Inﬂuence. Cambridge University
Press, Cambridge.
...
Huisman, M.E., Snijders, T.A.B., 2003. Statistical analysis of longitudinal network data
with changing composition. Sociological Methods & Research 32, 253–287.
Huisman, M., Steglich, C., 2008. Treatment of non-response in longitudinal network
data. Social Networks 30, 297–309.
Hummon, N.P., 2000. Utility and dynamic social networks. Social Networks 22,
221–249.
Jaccard, P., 1900. Contributions au problème de l’immigration post-glaciaire de la
ﬂore alpine. Bulletin de la Société Vaudoise des Sciences Naturelles 37, 547–579.
...
Katz, L., Proctor, C.H., 1959. The conﬁguration of interpersonal relations in
a group as a time-dependent stochastic process. Psychometrika 24, 317–
327.
Knecht, A., 2008. Friendship selection and friends’ inﬂuence. Dynamics of networks
and actor attributes in early adolescence. PhD Dissertation, University of Utrecht.
...
Journal of Mathematical Sociology 20, 1–21.
...
agent-based modelling. Annual Review of Sociology 28, 143–166.
...
dynamics in networks. Social Networks 17, 299–318.
Newcomb, T.M., 1962. Student peer-group inﬂuence. In: Sanford, N. (Ed.), The Amer-
...
dynamics given panel data: goodness-of-ﬁt tests.
...
dom graph models, Journal of Social Structure, 3.2. Available from: http://www.
...
bridge University Press, New York, pp. 215–247.
Snijders, T.A.B., 2006. Statistical Methods for Network Dynamics. In: Luchini, S.R.,.,
et al. (Eds.), Proceedings of the XLIII Scientiﬁc Meeting, Italian Statistical Society.
CLEUP, Padova (It.), pp. 281–296.
...
of networks and behavior. In: van Montfort, K., Oud, H., Satorra, A. (Eds.), Longitu-
dinal models in the behavioral and related sciences. Lawrence Erlbaum, Mahwah,
NJ, pp. 41–71.
...
1980. Jossey-Bass, San Francisco, pp. 392–412.
...
53, 261–282.
Wasserman, S., Pattison, P.E., 1996. Logit models and logistic regression for social net-
works: I. An introduction to Markov graphs and p∗. Psychometrika 61, 401–425.
